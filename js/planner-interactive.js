...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let selectedMonth = (storedMonth && monthKeys.includes(storedMonth)) ? storedMonth : today.toLocaleString('en-US', { month: 'short' }).toLowerCase();
  let selectedYear = (() => {
  ...
  if (monthTabsContainer) {
    ...
    yearSelect.addEventListener('change', (e) => {
      ...
      if (pageId === 'daily-focus') {
        ...
        if (currentSelected.getMonth() !== monthIndex || currentSelected.getFullYear() !== Number(selectedYear)) {
  ...
  function ensureMonthPopup() {
    ...
    monthPopup.innerHTML = `
      ...
      <div class="popup-calendar-grid">
        ${Array.from({ length: 35 }).map(() => '<div class="popup-cell"><div class="popup-date"></div></div>').join('')}
      </div>
    `;
  ...
  monthTabs.forEach(tab => {
    ...
    tab.addEventListener('click', (e) => {
      ...
      if (monthId) {
        ...
        console.debug('Month popup opened for', selectedMonth, selectedYear, 'at', popup.style.left, popup.style.top);
      }
    });
  });
  ...
  function syncDateFromMonthYear() {
    ...
    const d = new Date(`${selectedYear}-${String(monthIndex + 1).padStart(2, '0')}-01T00:00:00`);
  ...
  const editSelectors = [
    '.time-line', '.task-line', '.checkbox-line', '.reward-line',
    '.braindump-area', '.day-content', '.habit-label-cell',
    '.monthly-notes', '.lined-area', '.habit-name-cell',
    '.summary-lines', '.emergency-slot', '.cal-date', '.matrix-cell'
  ];
  ...
  function calculateExpenses() {
    ...
    sections.forEach(section => {
      ...
      if (subtotalEl) {
        subtotalEl.textContent = sectionTotal > 0 ? (sectionTotal % 1 === 0 ? sectionTotal.toString() : sectionTotal.toFixed(2)) : '';
      }
    ...
    if (grandTotalEl) {
      grandTotalEl.textContent = grandTotal > 0 ? (grandTotal % 1 === 0 ? grandTotal.toString() : grandTotal.toFixed(2)) : '';
    }
  }
  ...
  const toggleSelectors = [
    '.checkbox', '.selfcare-icon', '.done-badge',
    '.habit-check-cell', 'tbody td:not(.habit-name-cell)'
  ];
  ...
  function playPopSound(isChecking) {
    try {
      ...
      osc.type = 'sine';
      // Higher pitch for checking, lower pitch for unchecking
      const startFreq = isChecking ? 800 : 300;
      const endFreq = isChecking ? 150 : 100;
  ...
  togglesList.forEach((el, idx) => {
    ...
    el.addEventListener('click', () => {
      const isCheckboxLike = el.classList.contains('checkbox') ||
        el.classList.contains('habit-check-cell') ||
        el.tagName.toLowerCase() === 'td';

      const currentlyChecked = el.classList.contains(isCheckboxLike ? 'checked' : 'active');
  ...
  function safeSetItem(key, value) {
    try {
      ...
      if (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
        alert('Planner storage is full! Please clear some drawings or old text to save new changes.');
      }
    }
  }
  ...
  function saveToggleData() {
    ...
    togglesList.forEach(el => {
      const isChecked = el.classList.contains('checked') || el.classList.contains('active');
  ...
  function handleMobileResponsiveness() {
    ...
    if (isMobile) {
      const scaleFactor = (window.innerWidth - 20) / 816; // 816px is 8.5in
      pageContainer.style.transform = `scale(${scaleFactor})`;
      pageContainer.style.transformOrigin = 'top center';

      // Collapse the empty space left by the transform
      const unscaledHeight = pageContainer.offsetHeight || 1056;
      const scaledHeight = unscaledHeight * scaleFactor;
      pageContainer.style.marginBottom = `-${unscaledHeight - scaledHeight}px`;
    } else {
  ...
  function drawSegment(p1, p2, color, size) {
    ...
    ctx.lineWidth = size;
    ctx.moveTo(p1.x * (canvas.width / window.devicePixelRatio), p1.y * (canvas.height / window.devicePixelRatio));
    ctx.lineTo(p2.x * (canvas.width / window.devicePixelRatio), p2.y * (canvas.height / window.devicePixelRatio));
    ctx.stroke();
  }
  ...
  function eraseAt(pos) {
    ...
    for (let i = strokes.length - 1; i >= 0; i--) {
      ...
      if (stroke.type === 'sticker') {
        const dx = (stroke.x - pos.x) * (canvas.width / window.devicePixelRatio);
        const dy = (stroke.y - pos.y) * (canvas.height / window.devicePixelRatio);
        collided = Math.sqrt(dx * dx + dy * dy) < (eraserRadius + (stroke.size || 20) / 2);
      } else {
  ...
  canvas.addEventListener('pointermove', (e) => {
    if (!isDrawing || activeTool === 'cursor' || activeTool === 'sticker') return;
    const pos = getPos(e);
  ...
  function injectPenToolbar() {
    ...
    btnCursor.onclick = () => { playPopSound(true); setTool('cursor'); };
    ...
    btnPen.onclick = () => { playPopSound(true); setTool('pen'); };
    ...
    btnEraser.onclick = () => { playPopSound(true); setTool('eraser'); };
    ...
    btnSticker.onclick = () => { playPopSound(true); setTool('sticker'); };
    ...
    const themeAccent = getComputedStyle(document.documentElement).getPropertyValue('--accent-1').trim() || '#3a86ff';
    ...
    colors.forEach(c => {
      ...
      if (c.custom) {
        document.addEventListener('themeChanged', (e) => {
          const newAccent = getComputedStyle(document.documentElement).getPropertyValue('--accent-1').trim();
          if (newAccent) {
    ...
    toolbar.addEventListener('pointerdown', (e) => {
      ...
      if (e.target.closest('button') || e.target.closest('.pen-color-btn')) return;
    ...
    function setTool(toolName) {
      activeTool = toolName;
      document.querySelectorAll('.pen-tool-btn').forEach(btn => btn.classList.remove('active'));
  ...
  function updateThemeSpecificBackground() {
    ...
    const accentColor = style.getPropertyValue('--accent-1').trim() || '#7c6fa0';
