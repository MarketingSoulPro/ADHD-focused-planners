...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Strip dangerous HTML to prevent XSS from imported/restored content
  ...
  // Helper to determine date-based scope suffix depending on page type
  function getScopeSuffix() {
    ...
    } else if (pageId === "monthly-calendar" || pageId === "habit-tracker") {
      // Month-based scoping: YYYY-MM
      const y = d.getFullYear();
      const m = String(d.getMonth() + 1).padStart(2, "0");
      return `${y}-${m}`;
    } else {
      // Daily-scoped pages (daily-focus, brain-dump, dopamine-menu, expense-tracker, etc.)
      return selectedDate;
    }
  }

  // Storage key helper: daily/weekly/monthly keys for all pages, monthly fallback for migration
  function getStorageKey(type) {
    const suffix = getScopeSuffix();
    const newKey = `planner-${type}-${pageId}-${suffix}`;
    const oldKey = `planner-${type}-${pageId}-${selectedYear}-${selectedMonth}`;
  ...
  function getHolidayMapForRegion(region, year) {
    ...
    switch (region) {
      ...
      case "in":
        ...
        break;
      case "pk":
        addFixed(1, 5, "Kashmir Day", false);
        addFixed(2, 23, "Pakistan Day", false);
        addFixed(4, 1, "Labour Day", false);
        addFixed(7, 14, "Independence Day", false);
        addFixed(10, 9, "Iqbal Day", false);
        addFixed(11, 25, "Quaid-e-Azam Day / Christmas", false);
        break;
      case "de":
  ...
  editables.forEach((el, idx) => {
    ...
    // Auto-save on typing (blur or input) — immediate, no debounce
  ...
  function loadTextData() {
    ...
    editables.forEach((el) => {
      if (el.id === "global-deadlines-list") {
        el.innerHTML = sanitizeHtml(localStorage.getItem("planner-global-deadlines") || "");
        return;
      }
      el.innerHTML = sanitizeHtml(textData[el.dataset.persistId] || "");
    });
  }
  ...
  function saveAllData() {
    ...
    saveToggleData();
    saveStrokes();
    if (typeof window.saveExpenseData === "function") window.saveExpenseData();
    if (typeof window.saveReminderData === "function") window.saveReminderData();
    if (typeof window.saveDopamineData === "function") window.saveDopamineData();
    localStorage.setItem("selected-date", selectedDate);
    localStorage.setItem("selected-month", selectedMonth);
    localStorage.setItem("selected-year", selectedYear);
    if (window.BackupManager && window.BackupManager.syncToFolder) {
      window.BackupManager.syncToFolder();
    }
  }
  ...
  function loadAllData() {
    ...
    loadToggleData();
    loadStrokes();
    if (typeof window.loadExpenseData === "function") window.loadExpenseData();
    if (typeof window.loadReminderData === "function") window.loadReminderData();
    if (typeof window.loadDopamineData === "function") window.loadDopamineData();
  }
  ...
  let stickerSize = 24; // Default sticker size
  const emojiList = [
  ...
  function injectPenToolbar() {
    ...
    clearEmojisBtn.style.gridColumn = "span 5";
    ...
    sizeContainer.className = "sticker-size-container";
    ...
    sizeContainer.style.borderTop = "1px solid rgba(255, 255, 255, 0.1)";
    sizeContainer.style.paddingTop = "8px";

    const sizeLabelRow = document.createElement("div");
    sizeLabelRow.style.display = "flex";
    sizeLabelRow.style.justifyContent = "space-between";
    sizeLabelRow.style.alignItems = "center";
    
    const sizeLabel = document.createElement("span");
    sizeLabel.className = "pen-toolbar-label";
    sizeLabel.innerText = "Sticker Size";
    
    const sizeVal = document.createElement("span");
    sizeVal.style.fontSize = "9px";
    sizeVal.style.color = "rgba(255, 255, 255, 0.6)";
    sizeVal.style.fontWeight = "bold";
    sizeVal.innerText = `${stickerSize}px`;

    sizeLabelRow.appendChild(sizeLabel);
    sizeLabelRow.appendChild(sizeVal);
    sizeContainer.appendChild(sizeLabelRow);
    ...
    sizeSlider.style.accentColor = "var(--accent-1)";
    ...
    stickerPanel.appendChild(sizeContainer);
    ...
    toolbar.addEventListener("pointermove", (e) => {
      ...
      if (stickerPanel) {
        ...
        stickerPanel.style.bottom = "auto";
      }
    });
    ...
    function setTool(toolName) {
      ...
      // Clean up previous tool classes
      canvasContainer.classList.remove("tool-pen", "tool-eraser", "tool-sticker");
      ...
      if (toolName === "cursor") {
        ...
        canvasContainer.classList.add(`tool-${toolName}`);
...
`;
  ...
  if (todayBtn) {
    todayBtn.addEventListener("click", () => {
      ...
      localStorage.setItem("selected-date", todayStr);
  ...
  (function injectDateArrows() {
    const bar = document.querySelector(".week-date-bar");
    const weekBadge = document.getElementById("week-badge");
    const dateText = document.getElementById("date-display");
    if (!bar || !weekBadge || !dateText || document.getElementById("date-prev")) return;

    const left = document.createElement("button");
    left.className = "date-nav-arrow";
    left.id = "date-prev";
    left.textContent = "\u2190";
    left.setAttribute("aria-label", "Previous day");

    const right = document.createElement("button");
    right.className = "date-nav-arrow";
    right.id = "date-next";
    right.textContent = "\u2192";
    right.setAttribute("aria-label", "Next day");

    bar.insertBefore(left, weekBadge);
    dateText.parentNode.insertBefore(right, dateText.nextSibling);
    ...
    left.addEventListener("click", function () { shiftDate(-1); });
    right.addEventListener("click", function () { shiftDate(1); });
  })();

  // Planner name map for save confirmation
  const plannerNames = {
  ...
  // Save Data button handler with confirmation modal
  const saveDataBtn = document.getElementById("save-data-btn");
  if (saveDataBtn) {
    saveDataBtn.addEventListener("click", () => {
      const plannerName = plannerNames[pageId] || pageId;
      ...
      overlay.innerHTML = `
        <div class="save-confirm-modal">
          ...
          <div class="save-confirm-text">You are saving</div>
          <div class="save-confirm-name">${plannerName}</div>
          <div class="save-confirm-date">${dateStr}</div>
          <div class="save-confirm-actions">
            <button class="save-confirm-ok">CONFIRM</button>
            <button class="save-confirm-cancel">CANCEL</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);

      overlay.querySelector(".save-confirm-ok").addEventListener("click", () => {
        saveAllData();
        overlay.remove();
      ...
      overlay.querySelector(".save-confirm-cancel").addEventListener("click", () => {
        overlay.remove();
      });
  ...
  // ── 7. Save on page unload / visibility change ──
  const handlePageHide = () => saveAllData();
